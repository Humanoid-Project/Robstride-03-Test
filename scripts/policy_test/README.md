...
`l/r_knee_joint` 는 시뮬레이터에서는 정강이(4절링크 출력) 값인데, CAN
모터(ID4·10)가 재는 건 **크랭크** 축이다. 1입력→1출력 관계라 원리상 풀 수
있는 문제지만(닫힌형 해 존재, 중립비율 0.705·전체 구간 0.21~1.23로 비선형),
링크 길이가 코드 어디에도 없어 **이 변환도 아직 안 됐다**
(`robonex_description/scripts/robonex_serial.py`의 `SERIAL_GAINS`가
`r^2`으로 게인을 보정하는 것 자체가 두 각도가 다르다는 증거).
...
그래서 이 스크립트는:
- 관측의 무릎·발목 슬롯 6개를 실제 모터값으로 채우지 않고 **0.0** 으로 둔다.
- 크랭크 모터(무릎 pitch, 발목 upper/lower) 원시값은 참고용으로 따로
  표시하되 관측에 넣지 않는다.
- 정책이 내놓는 무릎·발목 action 은 "가상 출력축 목표각"으로만 표시한다.
  **그대로 CAN 프레임으로 만들어 모터에 보내면 안 된다** — 변환 방법이 없다.

이 6개를 제외한 6개 관절(hip_yaw/pitch/roll ×2, 힙만)은 직결이라 CAN 모터
하나가 그 관절 그대로라 (`hw-canbus.md` 매핑표) 실제 값을 그대로 쓴다.
