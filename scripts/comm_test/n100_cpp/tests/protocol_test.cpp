...
void testGroundFrameReordering() {
  ...
  for (int i = 0; i < 200 && !complete; ++i) {
    const n100::DriverStats current = driver.stats();
    if (current.ahrs_frames >= static_cast<std::uint64_t>(cycles) &&
        current.imu_frames >= static_cast<std::uint64_t>(cycles) &&
        current.ground_frames >= static_cast<std::uint64_t>(cycles)) {
      complete = true;
    } else {
      std::this_thread::sleep_for(std::chrono::milliseconds(5));
    }
